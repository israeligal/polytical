...
|---|---|---|---|---|
| 2026-06-18 | afbca46 | duel FULL sweep via 2 Sonnet subagents (browser + curl/SSR): logged-out + edge tokens + arenas + market button | 🔴→✅ | logged-out SSR renders the arena for anon (kicker+@commenter_qa+question), NO real-name leak (all @handle/bdi); /duel/garbage→friendly not-found; resolved arena (verdict/🏆crown/standings ✓✗/rematch) + open-multi (kicker/hail/VS/4 rows) no overflow; challenge button hidden logged-out; OG 200/png. **FIXED:** /duel/by-id/<bad\|missing> returned a blank 404 (route-handler notFound) → now redirects to /markets. Mobile viewport not emulable via resize_window (harness, not a bug). |
| 2026-06-18 | 16ea351 | duel settlement notifications + resolved-arena result (prod throwaway market, @commenter_qa vs @bigwhale_hr) | ✅ | resolveMarket → notifyDuelSettlements fired 2 correct duel_settled (commenter_qa "ניצחת בדו-קרב!" won, bigwhale_hr "הדו-קרב הוכרע" lost, right refChallengeId). /duel/[token] result: "כן" crowned (🏆 התשובה), verdict banner "ניצחת בדו-קרב!", standings ✓@commenter_qa(מאתגר/ת,את/ה)/✗@bigwhale_hr, rematch CTA, overflowX=false. /notifications shows duel_settled (gold accent) → click → /duel/by-id → result page. Migration 0034 applied to prod. 20 PGlite tests. Note: duel player also gets generic bet_won+market_resolved (accepted). SparkBurst celebration foreground-only (occluded QA tab). |
