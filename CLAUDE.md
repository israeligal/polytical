...
- **Groups / קואליציה** are private invite-only prediction clubs with their own auto-published הצעות, a **SANDBOXED** scoreboard, and per-motion מליאה. A group motion is just a `markets` row carrying `markets.groupId`; it resolves via a SEPARATE `resolveGroupMotion` that bumps only `group_members` counters — never global stats/cards/seasons. A coalition is a **global scope**: the header switcher sets an active-coalition cookie and the display reads scope to it via `coalitionScope({groupId})` (null → national `isNull`); `/g/[slug]` is management-only and motions render inline on the global feed. Sandbox/admin/cron/season reads stay hard-coded `isNull`. Load the `groups` skill before touching `app/lib/groups/`, the `/g/*` routes, the switcher, or group-scoped market reads; `docs/decisions/groups.md` + `docs/decisions/coalition-global-context.md` have the why.
...
- Schema is split: `app/lib/schema.ts` (core) + `app/lib/schema-votes.ts` (votes domain), re-exported from `schema.ts`; `drizzle.config.ts` lists both. Declare ALL indexes in-schema — `db:push` drops migration-only indexes.
