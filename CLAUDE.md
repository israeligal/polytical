...
- **Groups / קואליציה** are private invite-only prediction clubs with their own auto-published הצעות, a **SANDBOXED** scoreboard, and per-motion מליאה. A group motion is just a `markets` row carrying `markets.groupId`; it resolves via a SEPARATE `resolveGroupMotion` that bumps only `group_members` counters — never global stats/cards/seasons — and EVERY global market read filters `isNull(markets.groupId)`. Load the `groups` skill before touching `app/lib/groups/`, the `/g/*` routes, or group-scoped market reads; `docs/decisions/groups.md` has the why.
